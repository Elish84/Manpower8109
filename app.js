...
function openApp(sess){
  ...
  // In "gudud" mode we avoid collectionGroup("soldiers") because ANY denied "soldiers" doc anywhere in the DB
  // can cause the entire query to fail with "Missing or insufficient permissions".
  // Instead, we listen to the known paths forces/<forceId>/soldiers for each configured force and merge client-side.
  liveHint.textContent = "טוען נתונים בזמן אמת...";

  if(isGudud){
    const perForce = new Map(); // forceId -> rows[]
    const unsubs = [];

    const recompute = ()=>{
      allRows = [];
      for(const rows of perForce.values()) allRows.push(...rows);
      applyFilters();
      liveHint.textContent = `עודכן: ${new Date().toLocaleString("he-IL")}`;
    };

    for(const f of CFG.forces.filter(x=>x.id!=="gudud")){
      const qf = query(soldiersCol(f.id), orderBy("lastName"));
      const u = onSnapshot(qf, (snap)=>{
        const rows=[];
        snap.forEach(d=>rows.push({__id:d.id, ...d.data()}));
        perForce.set(f.id, rows);
        recompute();
      }, (err)=>{
        console.error(err);
        liveHint.textContent = "שגיאת הרשאות/חיבור (גדוד). בדוק Firestore Rules ו-Anonymous Auth.";
      });
      unsubs.push(u);
    }

    unsub = ()=>unsubs.forEach(fn=>{ try{ fn(); }catch(e){} });
  }else{
